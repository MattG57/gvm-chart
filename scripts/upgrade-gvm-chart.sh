...
helm upgrade gvm-release "$CHART_DIR" -f "$VALUES_FILE" -n default --set mongodb.auth.rootPassword="$MONGODB_ROOT_PASSWORD" #--dry-run --debug

echo " github-value-secret created in Defaut Namespace,  $CHART_DIR gvm-release upgrade initiated"
